...
# Write lock — prevents concurrent coroutines from interleaving transactions
# on the shared connection (single-writer serialization)
# Lazy-initialized to avoid cross-loop binding issues
...
async def replace_active_members(members: list[dict]):
    ...
    Uses write lock + transaction with rollback.
    Guards against accidental empty-wipe if previous count > 0.

    Each member dict must have: channel_id, membership_level
    Returns True if replaced, False if skipped (empty-wipe guard).
    """
    # Empty-wipe guard: if we have existing members but got an empty list,
    # skip to avoid mass-lockout from transient API issues
    if len(members) == 0:
        current = await count_active_members()
        if current > 0:
            logger.warning(
                "Skipping empty member wipe — %d existing members, received empty list "
                "(possible transient API issue). Manual override: truncate table directly.",
                current,
            )
            return False
